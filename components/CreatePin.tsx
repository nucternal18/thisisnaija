...
interface ICreatePin {
  ...
  setImageAsset: React.Dispatch<React.SetStateAction<any>>;
      ...
      <div className="flex ld:flex-row flex-col justify-center items-center bg-white lg:p-5 p-3 lg:w-4/5 w-full">
        ...
        <div className="flex flex-1 flex-col gap-6 lg:pl-5 mt-5 w-full">
          ...
          <input
            ...
            placeholder="Add a desitination link"
            className="outline-none text-2xl sm:text-3xl font-bold border-b-2 border-gray-200 p-2"
          />
          <div className="flex flex-col">
              <div>
                  ...
                  <select name="" id="" onChange={e => setCategory(e.target.value)} className="outline-none w-4/5 text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer">
                      ...
                      {categories.map((category) => <option key={category.name} value={category.name} className="bg-white text-base border-0 uppercase text-black">{category.name}</option>
                      )}
                  </select>
              </div>
